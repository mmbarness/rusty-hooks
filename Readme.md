...
```
    {
        ...
        "run_delay": u8, in case you want to bake in an extra delay between the last event and the script execution
    },
```

Every watch path provided in the json will be picked up by rusty-hooks, unless `enabled` is false, of course.

Tell rusty-hooks where to find the folder by passing it to the cli, like `--script-folder /home/<username>/scripts/`. The config file and the scripts need to be in the same folder. You can also pass a debug level like so: --log_level debug|info|error. The log level defaults to error. 
...
```
user@machine:~/dir/rusty-hooks$ target/release/rusty-hooks --script-folder ./scripts --log-level debug
...
[2023-03-31T13:17:09Z DEBUG rusty_hooks::scripts::load] deciding whether to attach script Ok("\"ingest_music.sh\"") to event type Modify
...
### Logging
If you're running this on Linux, logs will be written to home/*your_username*/rusty-hooks/logs/rusty-hooks.log.