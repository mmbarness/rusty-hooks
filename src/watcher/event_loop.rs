...
impl PathSubscriber {
    ...
    pub fn event_loop(
        ...
        handle.spawn(async move {
            ...
            let sub_path_str = subscription_path.to_str().unwrap_or("unable to pull string out of path buf");
            ...
            loop {
                ...
                if num_events_errors > 5 {
                    let new_unexpected_error:ThreadError = ThreadError::new_unexpected_error(format!("error while waiting on events to run out at watched path {}", sub_path_str));
    ...
    fn paths_overlap(event: notify::Event, sub_path_hash: u64) -> bool {
        event.paths.iter().fold(false, |overlap, cur_path| {
            ...
            let cur_path_parent = cur_path.ancestors().any(|ancestor| {
                let ancestor_path_str = ancestor.to_str().unwrap_or("unable to pull string out of path buf");
